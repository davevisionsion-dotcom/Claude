...
- Where the inspiration uses photography, adapt the SUBJECT to the client's topic (a laptop in the inspiration becomes a leadership-related photo when the post is about leadership). Keep the photo's size, position, crop, and any overlay/duotone/shadow treatment identical to the inspiration. Image source priority:
  ...
  2. Otherwise, if stock photos are ENABLED: place <img src="{{PHOTO_1}}" alt="..."> (then {{PHOTO_2}}, ...) with object-fit: cover, and list every token in "photos". Each search query must combine the CLIENT'S TOPIC with the INSPIRATION'S MOOD AND STYLE — e.g. "dark moody portrait rock climber", not just "rock climber". Then ALWAYS grade the placed photo with CSS so its lighting matches the inspiration's imagery: dark gradient overlays, color-wash layers with mix-blend-mode, vignettes, rim-light glows in the palette colors. A stock photo must never sit brighter or flatter than the inspiration's photo does.
...
- Brand marks: if a logo was provided, place an <img src="{{LOGO_SRC}}" alt="logo"> element (the application substitutes the real logo file into that exact token) sized and positioned the way the inspiration treats its logo/brand mark. Without a logo, you MAY set the brand name as a typographic mark — but ONLY where the inspiration has a brand mark in that spot. If the client's notes or instructions say "no logo", "no brand name", or similar, omit ALL brand marks — no logo, no wordmark, no brand name anywhere — even if the inspiration has one.
